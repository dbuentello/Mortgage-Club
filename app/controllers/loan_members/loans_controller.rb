class LoanMembers::LoansController < LoanMembers::BaseController
  ...
  def export_xml
    ...
    send_data xml, type: "text/plain; charset=UTF-8;", disposition: "attachment; filename=loan.xml"
  end

  def export_fnm
    loan = Loan.find(params[:id])
    fnm = ExportFnmService.new(loan).call

    send_data File.read(fnm), type: "text/plain; charset=UTF-8;", disposition: "attachment; filename=loan.fnm"
  end
