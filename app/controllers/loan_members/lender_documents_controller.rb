class LoanMembers::LenderDocumentsController < LoanMembers::BaseController
  ...
  def create
    return render json: {message: t("messages.errors.object_not_found", object: "Template")}, status: 500 unless template = LenderTemplate.find_by_id(params[:template_id])
  ...
  def set_document
    return render json: {message: t("messages.errors.object_not_found", object: "File")}, status: 500 unless @document = LenderDocument.find_by_id(params[:id])
  end
