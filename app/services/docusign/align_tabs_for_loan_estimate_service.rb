module Docusign
  class AlignTabsForLoanEstimateService
    HEADER_TABS = %w(
      ...
      loan_type_va loan_type_other loan_type_other_text loan_id rate_lock_no rate_lock_yes rate_lock_text
    )
    ...
    LOAN_TERM_TABS = %w(
      ...
      monthly_principal_interest prepayment_penalty prepayment_penalty_text balloon_payment balloon_payment_text
    )

    PROJECTED_PAYMENTS_TABS = %w(
      payment_calculation_text_1 payment_calculation_text_2 projected_principal_interest_1
    ...
    COSTS_AT_CLOSING_TABS = %w(estimated_closing_costs loan_costs other_costs lender_credits)

    LOAN_COSTS_TABS = %w(
      points application_fee underwriting_fee services_cannot_shop_total appraisal_fee credit_report_fee flood_determination_fee
      ...
      title_search points_text application_fee_text underwriting_fee_text appraisal_fee_text credit_report_fee_text
      flood_determination_fee_text flood_monitoring_fee_text tax_monitoring_fee_text tax_status_research_fee_text
      pest_inspection_fee_text survey_fee_text insurance_binder_text lenders_title_policy_text settlement_agent_fee_text title_search_text
    )
    ...
    OTHER_COSTS_TABS = %w(
      ...
      prepaid_interest_days prepaid_interest_rate prepaid_property_taxes_months initial_homeowner_insurance_per_month initial_homeowner_insurance_months
      intial_mortgage_insurance_per_month initial_mortgage_insurance_months initial_property_taxes_per_month initial_property_taxes_months
    )

    TOTAL_COSTS_TABS = %w(total_other_costs total_closing_costs total_loan_costs_and_other_costs)

    CALCULATING_CASH_TO_CLOSE_TABS = %w(total_closing_costs closing_costs_financed down_payment deposit funds_for_borrower seller_credits adjustments_and_other_credits)

    ADDITIONAL_INFORMATION_TABS = %w(
      lender_name lender_nmls_id loan_officer_name_1 loan_officer_nmls_id_1 loan_officer_email_1 loan_officer_phone_1
      mortgage_broker_name mortgage_broker_nmls_id loan_officer_name_2 loan_officer_nmls_id_2 loan_officer_email_2 loan_officer_phone_2
    )

    COMPARISONS_TABS = %w(in_5_years_total in_5_years_principal annual_percentage_rate total_interest_percentage)

    CONSIDERATIONS_TABS = %w(assumption_will_allow assumption_will_not_allow late_days late_fee_text servicing_service servicing_transfer)
    ...
    def align_projected_payments_tabs(field)
      case field[:name]
      when *%w(payment_calculation_text_1 projected_principal_interest_1 projected_mortgage_insurance_1 estimated_escrow_1
        estimated_total_monthly_payment_1 estimated_taxes_insurance_assessments estimated_taxes_insurance_assessments_text)
        field[:x_position] = 194
    ...
    def align_loan_costs_tabs(field)
      case field[:name]
      when *%w(origination_charges_total points application_fee underwriting_fee services_cannot_shop_total
              ...
              pest_inspection_fee survey_fee insurance_binder settlement_agent_fee title_search loan_costs_total)
        field[:x_position] = 240
      when *%w(points_text application_fee_text underwriting_fee_text appraisal_fee_text
              ...
              survey_fee_text insurance_binder_text lenders_title_policy_text settlement_agent_fee_text title_search_text)
        field[:x_position] = 43
      end
    end

    def align_other_costs_tabs(field)
      if %w(taxes_and_other_government_fees_total recording_fees_and_other_taxes transfer_taxes
            ...
            initial_mortgage_insurance initial_property_taxes other_total owner_title_policy).include? field[:name]
    ...
    def align_additional_information_tabs(field)
      case field[:name]
      when *%w(lender_name lender_nmls_id loan_officer_name_1 loan_officer_nmls_id_1 loan_officer_email_1 loan_officer_phone_1)
        field[:x_position] = 127
      when *%w(mortgage_broker_name mortgage_broker_nmls_id loan_officer_name_2 loan_officer_nmls_id_2 loan_officer_email_2 loan_officer_phone_2)
