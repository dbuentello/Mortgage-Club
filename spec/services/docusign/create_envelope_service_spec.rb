...
describe Docusign::CreateEnvelopeService do
  ...
  let(:secondary_template) { FactoryGirl.create(:template, name: "Servicing Disclosure") }
  # rubocop:disable all
  ...
  before(:each) do
    ...
    allow_any_instance_of(DocusignRest::Client).to receive(:create_envelope_from_composite_template).and_return(
      {
        "envelopeId" => "sample-envelope-id",
        "uri" => "",
        "statusDateTime" => "",
        "status" => ""
      }
