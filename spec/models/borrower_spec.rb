...
describe Borrower do
  ...
  describe "#destroy_completely" do
    context "with valid borrower params" do
      ...
      it "returns true" do
        expect(borrower.destroy_completely).to be_truthy
      end
    end

    context "with invalid borrower params" do
      it "does not destroy borrower" do
        borrower.user = nil
        expect(borrower.destroy_completely).to be_falsy
      end

      it "does not destroy user" do
        borrower.user = nil
        expect { borrower.destroy_completely }.to change(User, :count).by(0)
      end

      it "does not destroy borrower" do
        borrower.user = nil
        expect { borrower.destroy_completely }.to change(Borrower, :count).by(0)
      end
    end
  end
end
